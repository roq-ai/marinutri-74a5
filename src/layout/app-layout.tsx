...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['nutritionist'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<EmpresaInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/empresas` : null),
    () => getEmpresas({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Empresa',
      path: isTenantUser ? `/empresas/view/${data?.[0]?.id}` : '/empresas',
      entity: 'empresa',
