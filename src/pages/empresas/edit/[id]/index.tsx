...
import { getEmpresaById, updateEmpresaById } from 'apiSdk/empresas';
...
function EmpresaEditPage() {
  ...
  const handleSubmit = async (values: EmpresaInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateEmpresaById(id, values);
      mutate(updated);
      resetForm();
  ...
  const formik = useFormik<EmpresaInterface>({
    initialValues: data,
